...
- B turns into E (third byte of the third block, 34th total)
- O turns into V (fourth byte of the third block, 35th total)
- B turns into E (fifth byte of the third block, 36th total)
...
def bit_flipping(original_plaintext: bytes, modified_plaintext: bytes, ciphertext: bytes, changeiv=False, iv=None):
    ...
    if changeiv == True:
        # the first 16 bytes of our modified plaintext are wrong, so we need to change the iv
        wrong_positions = []
        ...
        # return a tuple containing the new ciphertext and the new iv
